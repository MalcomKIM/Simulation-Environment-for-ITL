...
public class TrajectoryPlanner : MonoBehaviour
{
	...
	Vector3 k_BaseLink_default_pose;
    ...
    void Start()
    {
		k_BaseLink_default_pose = new Vector3(0f, 0.63f, 0f);
    ...
    public void PublishJoints()
    {
		...
		var m_RobotPoseOffset = k_BaseLink_default_pose - m_NiryoOne.transform.Find("world/base_link").position;

        // Pick Pose
        request.pick_pose = new PoseMsg
        {
            position = (m_Target.transform.position + m_PickPoseOffset + m_RobotPoseOffset).To<FLU>(),
        ...
        {
            position = (m_TargetPlacement.transform.position + m_PickPoseOffset + m_RobotPoseOffset).To<FLU>(),
    ...
    void TrajectoryResponse(MoverServiceResponse response)
    {
        ...
        else
        {
			...
			try{
				GameObject p = GameObject.Find("Publisher");
				DemoPRS gc = p.GetComponent<DemoPRS>();
				gc.StateLog(1);
				gc.restart();
			}
			catch(Exception e){
				GameObject p = GameObject.Find("Publisher");
				CustomPRS gc = p.GetComponent<CustomPRS>();
				gc.StateLog(1);
				gc.restart();
			}
